                ...
                <div className="venue-left-details">
                    ...
                    <div className="venue-location-row">
                        ...
                        {venue.averageRating > 0 && (
                            <span className="venue-avg-rating">
                                <span style={{ color: '#ffc107', marginLeft: '15px' }}>★</span> {venue.averageRating.toFixed(1)} ({venue.numReviews} reviews)
                            </span>
                        )}
                    </div>
                ...
                <div className="venue-booking-sidebar">
                    <div className="booking-card">
                        ...
                        {venue.packages && venue.packages.length > 0 && (
                            <div className="packages-display-section" style={{ marginTop: '2rem', paddingTop: '1.5rem', borderTop: '1px solid #eee' }}>
                                <h4>Special Packages</h4>
                                {venue.packages.map((pkg, idx) => (
                                    <div key={idx} className="package-info-card" style={{ background: '#f9fafb', padding: '1rem', borderRadius: '8px', marginBottom: '1rem', border: '1px solid #f3f4f6' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                                            <strong style={{ color: '#1a237e' }}>{pkg.name}</strong>
                                            <span style={{ fontWeight: '600', color: '#10b981' }}>+ Rs. {pkg.price.toLocaleString()}</span>
                                        </div>
